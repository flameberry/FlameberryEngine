...
class CMakeRequirements:
    ...
    def __ConstructCMakeURL(cls):
        ...
        if system == "Windows":
            machineArchitecture = platform.machine().lower()

            # Handle special cases for Windows architecture
            if machineArchitecture in ["amd64", "x86_64"]:
                machineName = "x86_64"
            elif machineArchitecture in ["i386", "i686", "x86"]:
                machineName = "i386"
            elif machineArchitecture == "arm64":
                machineName = "arm64"
            else:
                return f"Unknown architecture: {machineArchitecture}"

            machineName = f"windows-{machineName}"
        elif system == "Darwin":
            machineName = (
                "macos-universal"
                if "arm64" in machineName or platform.mac_ver()[0] >= "10.13"
                else "macos10.10-universal"
            )
        elif system == "Linux":
            machineName = f"linux-{machineName}"
