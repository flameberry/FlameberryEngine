...
class VulkanSDKRequirements:
    ...
    def Validate(cls) -> bool:
        ...
        if (cls.__VulkanMinimumVersionRequired not in vulkanSDK):
            ColoredLogger.Logger.error(f'Vulkan SDK version installed not supported by Flameberry Engine: Required Version: {cls.__VulkanMinimumVersionRequired}x.x')
            return False
    ...
    def __ConstructVulkanSDKURL(cls) -> tuple[str, str]:
        URLS = {
            'Windows': (f'https://sdk.lunarg.com/sdk/download/{cls.__VulkanVersionPreferred}/windows/VulkanSDK-{cls.__VulkanVersionPreferred}-Installer.exe', '.exe'),
            'Darwin': (f'https://sdk.lunarg.com/sdk/download/{cls.__VulkanVersionPreferred}/mac/vulkansdk-macos-{cls.__VulkanVersionPreferred}.dmg', '.dmg'),
            'Linux': (f'https://sdk.lunarg.com/sdk/download/{cls.__VulkanVersionPreferred}/linux/vulkansdk-linux-x86_64-{cls.__VulkanVersionPreferred}.tar.xz', '.tar.xz')
        }
