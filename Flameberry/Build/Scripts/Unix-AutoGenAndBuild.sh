...
if command -v $cmakeCommand &>/dev/null; then
    cmake -Wno-dev -DCMAKE_BUILD_TYPE="Release" -S. -BIntermediate/Build/Auto
    cmake --build Intermediate/Build/Auto
else
    ...
    if [ -n "$cmakePath" ]; then
        ...
        "$cmakePath" -Wno-dev -DCMAKE_BUILD_TYPE="Release" -S. -BIntermediate/Build/Auto
        "$cmakePath" --build Intermediate/Build/Auto
    else
