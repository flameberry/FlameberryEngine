...
namespace Flameberry {
	...
	namespace Utils {
		...
		ReflectionFormatToShaderInputDataType(SpvReflectFormat format)
		{
			switch (format)
			{
	...
	Shader::Shader(const char* vertexShaderSpvPath,
		const char* fragmentShaderSpvPath)
	{
		// TODO: This is probably some odd behaviour to name the shader with just the
		// vertex shader's name
		const auto& stem = std::filesystem::path(vertexShaderSpvPath).stem().string();
		m_Name = stem.substr(0, stem.find('.'));

		// Vertex Shader
		std::vector<char> vertexShaderSpvBinaryCode =
			LoadShaderSpvCode(vertexShaderSpvPath);
		Reflect(vertexShaderSpvBinaryCode);
		m_VertexShaderModule = CreateVulkanShaderModule(vertexShaderSpvBinaryCode);

		// Fragment Shader
		std::vector<char> fragmentShaderSpvBinaryCode =
			LoadShaderSpvCode(fragmentShaderSpvPath);
		Reflect(fragmentShaderSpvBinaryCode);
		m_FragmentShaderModule =
			CreateVulkanShaderModule(fragmentShaderSpvBinaryCode);
	}
	...
	const ReflectionUniformVariableSpecification&
	Shader::GetUniform(const std::string& name) const
	{
		auto it = m_UniformFullNameToSpecification.find(name);
		FBY_ASSERT(it != m_UniformFullNameToSpecification.end(),
			"Uniform does not exist in shader: {}", name);
		return it->second;
	}

	const ReflectionDescriptorBindingSpecification&
	Shader::GetBinding(const std::string& name) const
	{
		auto it = m_DescriptorBindingVariableFullNameToSpecificationIndex.find(name);
		FBY_ASSERT(it != m_DescriptorBindingVariableFullNameToSpecificationIndex.end(),
			"Descriptor Binding does not exist in shader: {}", name);
	...
	{
		spv_reflect::ShaderModule reflectionShaderModule(shaderSpvBinaryCode.size(),
			shaderSpvBinaryCode.data());
...

		{
			...
			uint32_t count = 0;
			auto result =
				reflectionShaderModule.EnumeratePushConstantBlocks(&count, NULL);
			FBY_ASSERT(
				result == SPV_REFLECT_RESULT_SUCCESS,
				"Failed to Enumerate SPIRV-Reflect Push Constant Blocks for shader: {}",
				m_Name);
			std::vector<SpvReflectBlockVariable*> pcblocks(count);
			result = reflectionShaderModule.EnumeratePushConstantBlocks(
				&count, pcblocks.data());
			FBY_ASSERT(
				result == SPV_REFLECT_RESULT_SUCCESS,
				"Failed to Enumerate SPIRV-Reflect Push Constant Blocks for shader: {}",
			...
			for (uint32_t i = 0; i < count; i++)
			{
				...
				for (uint32_t j = 0; j < pcblocks[i]->member_count; j++)
				{
					...
					// Full name is used to ensure uniqueness of the uniform variable (even
					// though it is rare to overlap)
					m_UniformFullNameToSpecification[fullName] =
						ReflectionUniformVariableSpecification{
							.Name =
								fullName
									.c_str(), // Wondering if the full name should be used
				...
				m_PushConstantSpecifications.emplace_back(
					ReflectionPushConstantSpecification{
						.Name = pcblocks[i]->type_description->type_name,
						.Offset = pcblocks[i]->offset,
						.Size = absoluteSize,
						.VulkanShaderStage = (VkShaderStageFlagBits)
												 reflectionShaderModule.GetShaderStage(),
						.RendererOnly =
							Utils::HasPrefix(pcblocks[i]->type_description->type_name,
								FBY_SHADER_RENDERER_ONLY_PREFIX) });
			}
		}

		{
			...
			result =
				reflectionShaderModule.EnumerateDescriptorSets(&count, descSets.data());
			FBY_ASSERT(
				result == SPV_REFLECT_RESULT_SUCCESS,
				"Failed to Enumerate SPIRV-Reflect Push Constant Blocks for shader: {}",
				m_Name);
			...
			// Iterating through the reflection descriptor sets to store the binding
			// information needed to create a descriptor set
			for (uint32_t i = 0; i < count; i++)
			{
				m_DescriptorBindingSpecifications.reserve(
					m_DescriptorBindingSpecifications.size() + descSets[i]->binding_count);
				for (uint32_t j = 0; j < descSets[i]->binding_count; j++)
				{
					...
					if (binding->descriptor_type == SPV_REFLECT_DESCRIPTOR_TYPE_SAMPLER || binding->descriptor_type == SPV_REFLECT_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER || binding->descriptor_type == SPV_REFLECT_DESCRIPTOR_TYPE_SAMPLED_IMAGE || binding->descriptor_type == SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_IMAGE || binding->descriptor_type == SPV_REFLECT_DESCRIPTOR_TYPE_INPUT_ATTACHMENT)
					{
						isDescriptorTypeImage = true;
						// An image descriptor will be considered renderer only when it's name
						// has the following prefix
						rendererOnly =
							Utils::HasPrefix(binding->name, FBY_SHADER_RENDERER_ONLY_PREFIX);
					}
					else
					{
						isDescriptorTypeImage = false;
						// A buffer descriptor will be considered renderer only when it's TYPE
						// name (not the alias name) has the following prefix
						rendererOnly = Utils::HasPrefix(binding->type_description->type_name,
							FBY_SHADER_RENDERER_ONLY_PREFIX);
						// Is this the right way to deal with this? Being able to refer an
						// Uniform Buffer with it's alias If alias is not present then with
					...
					// Assumption here is that all the bindings are in order (Yet to find
					// out how would it affect the functioning)
					m_DescriptorBindingSpecifications.emplace_back(
						ReflectionDescriptorBindingSpecification{
							.Name = fullName,
							.Set = binding->set,
							...
							.VulkanShaderStage =
								(VkShaderStageFlags)reflectionShaderModule.GetShaderStage(),
							.RendererOnly = rendererOnly,
							.IsDescriptorTypeImage = isDescriptorTypeImage });

					// This unordered_map is stored only for convenience of setting the
					// Uniform Buffers/Images using their names in the shader It shouldn't
					// be accessed every frame
					m_DescriptorBindingVariableFullNameToSpecificationIndex[fullName] =
						m_DescriptorBindingSpecifications.size() - 1;
				}
				m_DescriptorSetSpecifications.emplace_back(
					ReflectionDescriptorSetSpecification{ .Set = descSets[i]->set,
						.BindingCount =
							descSets[i]->binding_count });
			}
		}
	}

	VkShaderModule
	Shader::CreateVulkanShaderModule(const std::vector<char>& shaderSpvBinaryCode)
	{
		VkShaderModuleCreateInfo shaderModuleCreateInfo{};
		shaderModuleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
		shaderModuleCreateInfo.codeSize = shaderSpvBinaryCode.size();
		shaderModuleCreateInfo.pCode =
			reinterpret_cast<const uint32_t*>(shaderSpvBinaryCode.data());

		const auto device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
		VkShaderModule vulkanShaderModule = VK_NULL_HANDLE;
		VK_CHECK_RESULT(vkCreateShaderModule(device, &shaderModuleCreateInfo, nullptr,
			&vulkanShaderModule));
	...
	void Shader::GetVertexAndFragmentShaderModules(
		VkShaderModule* outVertexShaderModule,
		VkShaderModule* outFragmentShaderModule) const
	{
		FBY_ASSERT(m_VulkanShaderStageFlags & (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT),
			"GetVertexAndFragmentShaderModules() should only be called for "
			"Shader made using Vertex and Fragment components both!");
	...
	{
		FBY_ASSERT(
			(m_VulkanShaderStageFlags > 0) && ((m_VulkanShaderStageFlags & (m_VulkanShaderStageFlags - 1)) == 0),
			"GetVulkanShaderModule() should only be called for Shader made using "
			"single component like Compute or Vertex or Fragment but not a "
			"combination of them!");
