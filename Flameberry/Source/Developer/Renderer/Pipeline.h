...
namespace Flameberry {
	...
	struct PipelineSpecification
	{
		...
		// And for those that don't need to be used in the shader replace them with
		// the equivalent Dummy Types
		...
		VkRect2D Scissor;
		bool BlendingEnable = false, DepthTestEnable = true, DepthWriteEnable = true,
			 DepthClampEnable = false, StencilTestEnable = false;
	...
	public:
		...
		Ref<DescriptorSetLayout> GetDescriptorSetLayout(uint32_t setIndex)
		{
			return m_DescriptorSetLayouts[setIndex];
		}

		ComputePipelineSpecification GetSpecification() const
		{
			return m_Specification;
		}
		VkPipelineLayout GetVulkanPipelineLayout() const { return m_PipelineLayout; }
