...
namespace Flameberry {
	...
	VulkanInstance::VulkanInstance()
	{
		...
		std::vector<const char*> vk_extensions(glfwExtensions,
			glfwExtensions + glfwExtensionCount);

		vk_extensions.push_back(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME);
		vk_extensions.push_back(
			VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME);
		if (enableValidationLayers)
			vk_extensions.push_back(VK_EXT_DEBUG_UTILS_EXTENSION_NAME);

		vk_create_info.enabledExtensionCount =
			static_cast<uint32_t>(vk_extensions.size());
		...
		if (enableValidationLayers)
		{
			...
			vk_debug_messenger_create_info.sType =
				VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
			...
			VK_CHECK_RESULT(CreateDebugUtilsMessengerEXT(
				&vk_debug_messenger_create_info, nullptr, &m_VkDebugMessenger));
		...
		VkDebugUtilsMessengerEXT* pMessenger)
	{
		...
		if (vkCreateDebugUtilsMessengerEXT)
			return vkCreateDebugUtilsMessengerEXT(m_VkInstance, pCreateInfo, pAllocator,
				pMessenger);
		else
