...
namespace Flameberry {
	...
	Skymap::Skymap(const std::filesystem::path& filepath)
	{
		...
		pixels =
			stbi_loadf(filepath.c_str(), &width, &height, &channels, STBI_rgb_alpha);
		FBY_ASSERT(pixels, "Failed to load skymap from (Pixels are empty): {}",
			filepath);
		...
		Ref<Image> equirectangularImage;
		{
			...
			equirectangularImage->TransitionLayout(
...

		{
			...
			m_CubemapImage->TransitionLayout(VK_IMAGE_LAYOUT_UNDEFINED,
		...
		pipelineSpec.Shader = CreateRef<Shader>(
			FBY_PROJECT_DIR
			"Flameberry/shaders/vulkan/bin/Flameberry_GenCubemap.comp.spv");

		Ref<ComputePipeline> cubemapGenerationPipeline =
			CreateRef<ComputePipeline>(pipelineSpec);
		...
		equirectangularImageInfo.imageLayout =
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		...
		cmdBufferSpecification.CommandPool =
			VulkanContext::GetCurrentDevice()->GetComputeCommandPool();
		...
		auto vulkanComputePipelineLayout =
			cubemapGenerationPipeline->GetVulkanPipelineLayout();
		auto vulkanComputePipeline = cubemapGenerationPipeline->GetVulkanPipeline();
		auto vulkanDescSet = descSet->GetVulkanDescriptorSet();

		vkCmdBindPipeline(vulkanCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
			vulkanComputePipeline);
		vkCmdBindDescriptorSets(vulkanCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
			vulkanComputePipelineLayout, 0, 1, &vulkanDescSet, 0,
			0);
		...
		vkQueueSubmit(VulkanContext::GetCurrentDevice()->GetComputeQueue(), 1,
			&submitInfo, VK_NULL_HANDLE);

		// TODO: Remove this in the future
		VulkanContext::GetCurrentDevice()->WaitIdle();

		m_CubemapImage->TransitionLayout(VK_IMAGE_LAYOUT_GENERAL,
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
		...
		skymapDescSetSpec.Layout = DescriptorSetLayout::CreateOrGetCached(
			DescriptorSetLayoutSpecification{ .Bindings = { descBinding } });
		;
