...
namespace Flameberry {
		...
		: m_Shader(shader)
	{
		...
		// Currently the push constant blocks which are material specific (i.e. not
		// Renderer Only) are accumulated and the combined data is stored together As
		// of now I haven't encountered a case where multiple material specific push
		// constant blocks are used
		for (const auto& specification : m_Shader->GetPushConstantSpecifications())
		{
			// This is to remind the developer to implement this case properly when such
			// a shader is written
			FBY_ASSERT(!m_PushConstantBufferSize,
				"Multiple push constants not handled correctly yet!");
			...
			// This variable is purely for calculating the starting offset of the push
			// constant block
		...
		const auto& reflectionDescriptorSets =
			m_Shader->GetDescriptorSetSpecifications();
		const auto& descriptorBindings =
			m_Shader->GetDescriptorBindingSpecifications();
		...
		for (const auto& reflectionDescSet : reflectionDescriptorSets)
		{
			...
			{
				// This ensures that the Renderer Only Descriptor Sets are not handled or
				// stored by the Material class
			...
			if (vulkanDescSetBindings.size())
			{
				...
				// Only set this the first time any set is created which will inherently
				// store the start set index
				if (m_StartSetIndex == -1)
					m_StartSetIndex = reflectionDescSet.Set;

				// This will assert the assumption that all the material specific
				// descriptor sets will have sequential set indices
				FBY_ASSERT(reflectionDescSet.Set - m_StartSetIndex == m_DescriptorSets.size() - 1,
					"Material specific descriptor sets do not have sequentialy "
					"set indices!");
			}
		}
	}
