...
namespace Flameberry {
	...
	static void PopulateVulkanPushConstantRanges(
		std::vector<VkPushConstantRange>& outPushConstantRanges,
		const Ref<Shader>& shader)
	{
		...
		{
			if (auto it = pcOffsetToIndex.find(specification.Offset);
				it != pcOffsetToIndex.end())
			{
				FBY_ASSERT(!(specification.VulkanShaderStage & outPushConstantRanges[it->second].stageFlags),
					"Push constant blocks within the same shader stage must not "
					"have the same offset!");
				// TODO: Maybe make this be just a warning, so that the flexibility still
				// remains
				FBY_ASSERT(specification.Size == outPushConstantRanges[it->second].size,
					"Push constant blocks between different shader stages having "
					"the same offset must have the same size! (The assumption "
					"here is that both represent the same push constant block)");
				outPushConstantRanges[it->second].stageFlags |=
					specification.VulkanShaderStage;
			}
			else
			{
				outPushConstantRanges.emplace_back(VkPushConstantRange{
					.offset = specification.Offset,
					.size = specification.Size,
					.stageFlags = (VkShaderStageFlags)specification.VulkanShaderStage });
				pcOffsetToIndex[specification.Offset] =
					(uint32_t)outPushConstantRanges.size() - 1;
			}
		}
	}

	static void PopulateVulkanDescriptorSetLayouts(
		std::vector<VkDescriptorSetLayout>& outVulkanDescriptorSetLayouts,
		std::vector<Ref<DescriptorSetLayout>>& outDescriptorSetLayouts,
		const Ref<Shader>& shader)
	{
		// Creating the Descriptor Set Layouts required for creating the Pipeline
		// based on the Shader Reflection Data
		const auto& reflectionDescriptorSets =
			shader->GetDescriptorSetSpecifications();
		...
		for (const auto& reflectionDescSet : reflectionDescriptorSets)
		{
			...
			{
				DescriptorSetLayoutSpecification layoutSpecification{
					vulkanDescSetBindings
				};
				auto& layout = outDescriptorSetLayouts.emplace_back(
					DescriptorSetLayout::CreateOrGetCached(layoutSpecification));
	...
	void Pipeline::CreatePipeline()
	{
		...
		PopulateVulkanPushConstantRanges(vulkanPushConstantRanges,
			m_Specification.Shader);

		std::vector<VkDescriptorSetLayout> vulkanDescriptorSetLayouts;
		PopulateVulkanDescriptorSetLayouts(vulkanDescriptorSetLayouts,
		...
		vk_pipeline_layout_create_info.setLayoutCount =
			(uint32_t)vulkanDescriptorSetLayouts.size();
		vk_pipeline_layout_create_info.pSetLayouts =
			vulkanDescriptorSetLayouts.data();
		vk_pipeline_layout_create_info.pushConstantRangeCount =
			(uint32_t)vulkanPushConstantRanges.size();
		vk_pipeline_layout_create_info.pPushConstantRanges =
			vulkanPushConstantRanges.data();

		VK_CHECK_RESULT(vkCreatePipelineLayout(
			device, &vk_pipeline_layout_create_info, nullptr, &m_PipelineLayout));

		// Creating Pipeline
		VkShaderModule vertexModule, fragmentModule;
		m_Specification.Shader->GetVertexAndFragmentShaderModules(&vertexModule,
			&fragmentModule);

		VkPipelineShaderStageCreateInfo vk_pipeline_vertex_shader_stage_create_info{};
		vk_pipeline_vertex_shader_stage_create_info.sType =
			VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
		vk_pipeline_vertex_shader_stage_create_info.stage =
		...
		VkPipelineShaderStageCreateInfo
			vk_pipeline_fragment_shader_stage_create_info{};
		vk_pipeline_fragment_shader_stage_create_info.sType =
			VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
		vk_pipeline_fragment_shader_stage_create_info.stage =
		...
		VkPipelineShaderStageCreateInfo vk_shader_stages_create_infos[2] = {
			vk_pipeline_vertex_shader_stage_create_info,
			vk_pipeline_fragment_shader_stage_create_info
		};
		...
		pipelineDepthStencilStateCreateInfo.depthTestEnable =
			(VkBool32)m_Specification.DepthTestEnable;
		pipelineDepthStencilStateCreateInfo.depthWriteEnable =
			(VkBool32)m_Specification.DepthWriteEnable;
		pipelineDepthStencilStateCreateInfo.depthCompareOp =
			m_Specification.DepthCompareOp;
		...
		pipelineDepthStencilStateCreateInfo.stencilTestEnable =
			(VkBool32)m_Specification.StencilTestEnable;
		...
		// Create the vertex attribute description structs based on the
		// m_PipelineSpec.VertexLayout
		VkVertexInputAttributeDescription
			vertexAttributeDescriptions[m_Specification.VertexLayout.size()];
		...
		for (uint32_t loc = 0; loc < m_Specification.VertexLayout.size(); loc++)
		{
			...
			if (dataType < ShaderDataType::Dummy1)
			{
				...
				vertexAttributeDescriptions[idx].format =
					ShaderDataTypeToFormat(dataType);
		...
		// Fill the Vertex Binding Description purely based upon the vertex layout
		// provided
		VkVertexInputBindingDescription vertexInputBindingDescription{};
		vertexInputBindingDescription.binding = 0;
		vertexInputBindingDescription.stride =
			offset; // The final offset will turn out to be the total stride
		...
		pipelineVertexInputStateCreateInfo.vertexBindingDescriptionCount =
			vertexInputBindingDescription.stride ? 1 : 0;
		pipelineVertexInputStateCreateInfo.pVertexBindingDescriptions =
			&vertexInputBindingDescription;
		pipelineVertexInputStateCreateInfo.vertexAttributeDescriptionCount =
			idx; // Index will represent Size after the last increment
		...
		pipelineInputAssemblyStateCreateInfo.topology =
			m_Specification.PrimitiveTopology;
		...
		if (m_Specification.Samples > 1)
		{
			...
			pipelineMultisampleStateCreateInfo.rasterizationSamples =
				(VkSampleCountFlagBits)m_Specification.Samples;
		}
		else
		{
		...
		vk_graphics_pipeline_create_info.renderPass =
			m_Specification.RenderPass->GetRenderPass();
	...
	ComputePipeline::ComputePipeline(
		const ComputePipelineSpecification& pipelineSpec)
		: m_Specification(pipelineSpec)
	{
		const auto device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
		...
		VkShaderModule computeShaderModule =
			m_Specification.Shader->GetVulkanShaderModule();

		VkPipelineShaderStageCreateInfo pipelineComputeShaderStageCreateInfo{};
		pipelineComputeShaderStageCreateInfo.sType =
			VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
		...
		VK_CHECK_RESULT(vkCreateComputePipelines(device, VK_NULL_HANDLE, 1,
