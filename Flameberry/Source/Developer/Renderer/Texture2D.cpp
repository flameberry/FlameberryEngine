...
namespace Flameberry {
	...
	std::unordered_map<std::string, Ref<Texture2D>>
		Texture2D::s_TextureCacheDirectory;

	Texture2D::Texture2D(const std::string& texturePath, bool canGenerateMipMaps,
		VkSampler sampler)
	{
		...
		{
			pixels = stbi_loadf(texturePath.c_str(), &width, &height, &channels,
				STBI_rgb_alpha);
			mipLevels = 1;
		...
		{
			pixels = stbi_load(texturePath.c_str(), &width, &height, &channels,
				STBI_rgb_alpha);
			mipLevels =
		...
		SetupTexture(pixels, width, height, imageSize,
			canGenerateMipMaps ? mipLevels : 1, format, sampler);
		stbi_image_free(pixels);
	}

	Texture2D::Texture2D(const void* data, const float width, const float height,
		const uint8_t bytesPerChannel, VkFormat format,
		bool canGenerateMipMaps, VkSampler sampler)
	{
		FBY_ASSERT(data, "Texture pixels are empty!");
		uint32_t mipLevels =
			canGenerateMipMaps ? static_cast<uint32_t>(
	...
	void Texture2D::SetupTexture(const void* data, const float width,
		const float height, const float imageSize,
		const uint16_t mipLevels, const VkFormat format,
		const VkSampler sampler)
	{
		...
		m_TextureImageSpecification.MemoryProperties =
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
		m_TextureImageSpecification.ViewSpecification.AspectFlags =
		...
		m_TextureImage->TransitionLayout(VK_IMAGE_LAYOUT_UNDEFINED,
		...
		if (m_TextureImageSpecification.MipLevels > 1)
			...
			m_TextureImage->TransitionLayout(VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
		...
		if (sampler == VK_NULL_HANDLE)
		{
			...
			vkGetPhysicalDeviceProperties(VulkanContext::GetPhysicalDevice(),
				&properties);
			...
			VK_CHECK_RESULT(
				vkCreateSampler(device, &sampler_info, nullptr, &m_VkTextureSampler));
		}
		else
	...
	Texture2D::~Texture2D()
	{
		...
		if (m_DescriptorSet != VK_NULL_HANDLE)
			vkFreeDescriptorSets(device,
				VulkanContext::GetCurrentGlobalDescriptorPool()
	...
	VkDescriptorSet Texture2D::CreateOrGetDescriptorSet()
	{
		if (m_DescriptorSet == VK_NULL_HANDLE)
		{
			VulkanContext::GetCurrentGlobalDescriptorPool()->AllocateDescriptorSet(
				&m_DescriptorSet, s_DescriptorLayout->GetLayout());
	...
	Ref<Texture2D> Texture2D::TryGetOrLoadTexture(const std::string& texturePath)
	{
		...
		{
			FBY_TRACE("Successfully retrieved vulkan texture from cache: {}",
				texturePath);
		...
		else
		{
			...
			FBY_TRACE("Texture not found in cache! Loading texture from disk: {}",
				texturePath);
			return texture;
		}
	}
	...
	void Texture2D::InitStaticResources()
	{
...

		{
			...
			vkCmdPipelineBarrier(commandBuffer, sourceStageFlags, destinationStageFlags,
		...
		VK_CHECK_RESULT(
			vkCreateSampler(device, &sampler_info, nullptr, &s_DefaultSampler));
		...
		emptyDescSetLayoutSpec.Bindings[0].descriptorType =
		...
		s_DescriptorLayout =
			DescriptorSetLayout::CreateOrGetCached(emptyDescSetLayoutSpec);
