...
namespace Flameberry {
	...
	public:
		...
		static void WritePixelFromImageToBuffer(VkBuffer buffer, VkImage image,
		...
		CreateShaderModule(const std::vector<char>& compiledShaderCode);
		static VkSampleCountFlagBits
		GetMaxUsableSampleCount(VkPhysicalDevice physicalDevice);
		static uint32_t
		GetValidMemoryTypeIndex(VkPhysicalDevice physicalDevice, uint32_t typeFilter,
			VkMemoryPropertyFlags vk_memory_property_flags);
		static bool HasStencilComponent(VkFormat format);
		static VkFormat
		GetSupportedFormat(VkPhysicalDevice physicalDevice,
