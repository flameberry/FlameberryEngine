...
namespace Flameberry {
		...
		: m_Specification(specification)
	{
		...
		FBY_ASSERT(m_PrimaryMonitorWidth && m_PrimaryMonitorHeight,
			"Monitor size not initialized!");

		glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);

		m_Window = glfwCreateWindow(m_Specification.Width, m_Specification.Height,
			m_Specification.Title, NULL, NULL);
		FBY_ASSERT(m_Window, "GLFW window is null!");
		FBY_INFO("Created GLFW window of title '{}' and dimensions ({}, {})",
	...
	void VulkanWindow::Init()
	{
		...
		{
			Platform::TitlebarNative::CreateForGLFWwindow(
				m_Window, m_Specification.TitlebarHeight);
	...
	void VulkanWindow::SwapBuffers()
	{
		...
		VkResult queuePresentStatus = m_SwapChain->SubmitCommandBuffer(
			device->GetCommandBuffer(Renderer::RT_GetCurrentFrameIndex()));

		// TODO: This code should be enabled when ensured that all the resources that
		// depend upon the swapchain are also updated if (queuePresentStatus ==
		// VK_ERROR_OUT_OF_DATE_KHR || queuePresentStatus == VK_SUBOPTIMAL_KHR)
		//     m_SwapChain->Invalidate();
	}
	...
	{
		if (!m_Specification.NativeTitlebar)
			Platform::TitlebarNative::InvalidateFrameAndContentFrameRect(
				m_Window, m_Specification.TitlebarHeight);
