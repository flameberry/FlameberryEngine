...
namespace Flameberry {
	...
	void PhysicsEngine::Init(int numThreads)
	{
		s_Foundation = PxCreateFoundation(
			PX_PHYSICS_VERSION, s_DefaultAllocatorCallback, s_DefaultErrorCallback);
		FBY_ASSERT(s_Foundation, "Failed to create PxFoundation object!");

		bool recordMemoryAllocations = true;

		s_Physics =
			PxCreatePhysics(PX_PHYSICS_VERSION, *s_Foundation, s_TolerancesScale,
		...
		s_Cooking = PxCreateCooking(PX_PHYSICS_VERSION, *s_Foundation,
			physx::PxCookingParams(s_TolerancesScale));
