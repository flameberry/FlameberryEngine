...
namespace fbentt {
	...
	public:
		...
		public:
			registry_view() {}
			registry_view(const registry* reg, const pool_data* pool)
				: ref_registry(reg), ref_pool(pool) {}
		...
		public:
			...
			{
			public:
				iterator(const registry* reg, const pool_data* pool, uint32_t index)
					: ref_registry(reg), ref_pool(pool), index(index) {}
		...
		public:
			...
			registry_group(const registry* reg, const pool_data* pool)
				: ref_registry(reg), ref_pool(pool), begin_index(0), end_index(0)
			{
	...
	public:
		 ...
		 */
		template <typename Fn>
		void for_each_component(entity entity, Fn&& _Fn)
		{
		...
		decltype(auto) try_get(const entity& entity) const
		{
		...
		template <typename Type, typename... MoreTypes>
		void erase(const entity& entity) const
		{
