...
namespace Flameberry {
	...
	public:
		...
		static double GetExecutionTime(const std::string& scopeName)
		{
			return s_ScopeExecutionTimes[scopeName];
		}
	...
	#define FBY_DISPLAY_SCOPE_DETAILS_IMGUI() \
		Flameberry::Profiler::DisplayScopeDetailsImGui()
#else
	#define FBY_PROFILE_SCOPE(scopeName) Flameberry::Profiler profiler(scopeName);
	#define FBY_DISPLAY_SCOPE_DETAILS_IMGUI() \
		Flameberry::Profiler::DisplayScopeDetailsImGui()
