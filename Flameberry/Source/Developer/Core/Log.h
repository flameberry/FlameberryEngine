...
/// Xcode doesn't support integrated terminal output by default, so the escape
/// characters for coloring output are printed out as it is, which just creates
/// a mess, so the macro `FBY_XCODE_PROJ` is defined by CMake, if the cmake
/// generator is `Xcode` and the macros are defined as `""`
...
namespace Flameberry {
	...
	public:
		...
		inline static void SetCoreLogger(const Ref<Logger>& logger)
		{
			s_CoreLogger = logger;
		}
	...
	public:
		...
		void critical(const T& message, const Args&... args)
		{
			if (m_CurrentLogLevel <= LogLevel::CRITICAL)
				fmt::print(fmt::fg(fmt::terminal_color::red) | fmt::bg(fmt::color::yellow),
					"{}: {}\n", GetPrefix(LogLevel::CRITICAL),
					fmt::format(message, args...));
		}

		template <typename T, typename... Args>
		void log_assert(const char* file, int line, const T& message,
			const Args&... args)
		{
			std::string msg = fmt::format(message, args...);
			fmt::print(fmt::fg(fmt::terminal_color::red),
				"{} [ASSERT] Assertion failed: {} (file: {}, line: {})",
				GetCurrentTimeString(), msg, file, line);
		}
		...
		void log_assert(const char* file, int line)
		{
			fmt::print(fmt::fg(fmt::terminal_color::red),
				"{} [ASSERT] Assertion failed: (file: {}, line: {})",
				GetCurrentTimeString(), file, line);
		}
...
#define FBY_CRITICAL(...) \
	Flameberry::Logger::GetCoreLogger()->critical(__VA_ARGS__)
