...
namespace Flameberry {
	ContentBrowserPanel::ContentBrowserPanel()
		: m_CurrentDirectory(
			  Project::GetActiveProject()
				  ->GetConfig()
				  .AssetDirectory)
		, // Getting Asset Directory via this method to
		...
		, m_VkTextureSampler(Texture2D::GetDefaultSampler())
	{
		for (const auto& path : g_IconPaths)
			m_IconTextures.emplace_back(
				CreateRef<Texture2D>(path.c_str(), m_VkTextureSampler));
	}

	ContentBrowserPanel::~ContentBrowserPanel() {}

	void ContentBrowserPanel::RecursivelyAddDirectoryNodes(
		const std::filesystem::directory_entry& parent,
		const std::filesystem::directory_iterator& iterator)
	{
		...
		const int treeNodeFlags =
			(is_selected ? ImGuiTreeNodeFlags_Selected : 0) | ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_SpanFullWidth | ImGuiTreeNodeFlags_FramePadding | (is_leaf ? ImGuiTreeNodeFlags_Leaf : 0);
		...
		ImGui::PushStyleColor(ImGuiCol_Text,
			ImVec4{ textColor, textColor, textColor, 1.0f });
		...
		ImGui::Image(
			reinterpret_cast<ImTextureID>(
				m_IconTextures[FileTypeIndex::FOLDER]->CreateOrGetDescriptorSet()),
			ImVec2{ ImGui::GetTextLineHeight() + framePaddingY,
				ImGui::GetTextLineHeight() + framePaddingY });
		ImGui::SameLine();
		...
		if (is_opened)
		{
			for (auto& directory : iterator)
			{
				...
				{
					RecursivelyAddDirectoryNodes(
						directory, std::filesystem::directory_iterator{ directory });
				}
			}
			ImGui::TreePop();
		}
	...
	bool ContentBrowserPanel::IsPathInHierarchy(
		const std::filesystem::path& key, const std::filesystem::path& parent)
	{
	...
	void ContentBrowserPanel::OnUIRender()
	{
		...
		m_SecondChildSize =
			ImGui::GetContentRegionAvail().x - m_FirstChildSize - 8.0f;
		...
		ImGui::BeginChild("##FileStructurePanel", ImVec2(m_FirstChildSize, 0), false,
		...
		for (auto& directory : std::filesystem::directory_iterator(
				 Project::GetActiveProject()->GetConfig().AssetDirectory))
		{
			if (directory.is_directory())
			{
				RecursivelyAddDirectoryNodes(
		...
		ImGui::GetWindowDrawList()->AddRectFilledMultiColor(
			pMin, pMax, IM_COL32(5, 5, 5, 0), IM_COL32(5, 5, 5, 140),
			IM_COL32(5, 5, 5, 140), IM_COL32(5, 5, 5, 0));
		...
		ImGui::BeginChild("##ContentBrowserTopBar",
			ImVec2(m_SecondChildSize, topChildHeight), false,
			ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar);
		ImGui::PopStyleVar();
		...
		if (ImGui::ImageButton(
				reinterpret_cast<ImTextureID>(
					m_IconTextures[FBY_BACK_ARROW_ICON]->CreateOrGetDescriptorSet()),
				ImVec2{ arrowSize, arrowSize })
			&& m_CurrentDirectory != "Content")
			m_CurrentDirectory = m_CurrentDirectory.parent_path();
		ImGui::SameLine(0.0f, 0.0f);
		if (ImGui::ImageButton(
				reinterpret_cast<ImTextureID>(m_IconTextures[FBY_FORWARD_ARROW_ICON]
												  ->CreateOrGetDescriptorSet()),
				ImVec2{ arrowSize, arrowSize })
			&& m_CurrentDirectory != "Content")
			m_CurrentDirectory = m_CurrentDirectory.parent_path();
		...
		UI::InputBox("##ContentBrowserSearchBar", 150.0f, m_SearchInputBuffer, 256,
		...
		const float totalIconWidth =
			arrowSize + 2.0f * style.FramePadding.x + style.ItemSpacing.x;
		ImGui::SameLine();
		ImGui::SetCursorPosX(ImGui::GetWindowSize().x - totalIconWidth);

		ImGui::ImageButton(
			"##ContentBrowserPanelSettingsButton",
			reinterpret_cast<ImTextureID>(
				m_IconTextures[FBY_SETTINGS_ICON]->CreateOrGetDescriptorSet()),
			ImVec2(arrowSize, arrowSize), ImVec2(0, 0), ImVec2(1.0f, 1.0f));
		...
		if (ImGui::BeginPopup("##ContentBrowserPanelSettingsPopup"))
		{
			...
			ImGui::DragInt("Thumbnail Size", (int32_t*)&m_ThumbnailSize, 1.0f, 50,
				400);
			ImGui::PopItemWidth();
			ImGui::EndPopup();
		}
		...
		// Separator
		ImVec2 pos = ImVec2(ImGui::GetWindowPos().x,
			ImGui::GetWindowHeight() + ImGui::GetWindowPos().y);
		ImGui::GetWindowDrawList()->AddLine(
			pos, ImVec2{ pos.x + ImGui::GetWindowWidth(), pos.y }, 0xff141414, 2.0f);

		ImGui::EndChild();

		ImGui::SetNextWindowPos(pos);
		ImGui::BeginChild("##Contents", ImVec2(m_SecondChildSize, bottomChildHeight),
			false,
			ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_AlwaysAutoResize);
		ImGui::PopStyleVar();
			...
			std::filesystem::directory_iterator{ m_CurrentDirectory })
		{
			...
			{
				// TODO: Maybe some optimisation to not search again if the input string
				// is same
				const int index =
					Algorithm::KmpSearch(filePath.filename().replace_extension().c_str(),
						m_SearchInputBuffer, true);
				if (index == -1)
					continue;
			}
			...
			if (UI::ContentBrowserItem(filePath, m_ThumbnailSize, thumbnail, itemSize,
					!isFileSupported)
				&& isDirectory)
		...
		if (ImGui::BeginPopupContextWindow((const char*)__null, m_PopupFlags))
		{
			if (ImGui::BeginMenu(ICON_LC_PLUS "\tCreate"))
			{
				if (ImGui::MenuItem(ICON_LC_DRIBBBLE "\tMaterial"))
				{
					...
					MaterialAssetSerializer::Serialize(
						mat, (m_CurrentDirectory / "NewMaterial.fbmat").c_str());
				}
				ImGui::EndMenu();
			}
			if (ImGui::MenuItem(ICON_LC_EXTERNAL_LINK "\tOpen In Finder"))
				Platform::OpenInExplorerOrFinder(
					(Project::GetActiveProjectDirectory() / m_CurrentDirectory)
						.string()
						.c_str());
			ImGui::EndPopup();
		}
	...
	void ContentBrowserPanel::UI_CurrentPathBar()
	{
		...
		const float totalIconWidth =
			14.0f + 2.0f * style.FramePadding.x + 2.0f * style.ItemSpacing.x;
		const float currentPathItemWidth =
			ImGui::GetContentRegionAvail().x - totalIconWidth;

		const ImVec2 cursorPositionRelativeStart = ImGui::GetCursorScreenPos();
		const ImRect clipRect(
			cursorPositionRelativeStart,
			cursorPositionRelativeStart + ImVec2(currentPathItemWidth, ImGui::GetFontSize() + 2.0f * style.FramePadding.y));
		...
		ImGui::GetWindowDrawList()->AddRectFilled(
			...
			ImGui::ColorConvertFloat4ToU32(Theme::FrameBg), style.FrameRounding);
		...
		// Later on path can be "Content/Textures", "Content/Textures/Texture1",
		// "Content/Textures/Texture1/Texture2" etc.
		while (!currentPath.empty())
		{
			std::size_t position =
				currentPath.find_first_of(std::filesystem::path::preferred_separator);
			...
			const ImRect buttonRect(
				cursorScreenPos - ImVec2(style.FramePadding.x, 0.0f),
			...
			const bool hovered =
				ImGui::IsWindowHovered() && ImGui::IsMouseHoveringRect(buttonRect.Min, buttonRect.Max, false);
