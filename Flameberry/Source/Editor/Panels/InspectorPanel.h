...
namespace Flameberry {
	...
	public:
		...
		void SetSelectionContext(const fbentt::entity& selectionContext)
		{
	...
	private:
		Ref<fbentt::registry> GetContextRegistry() const
		{
			return m_Context->m_Registry;
		}

	private:
		static constexpr ImGuiTableFlags s_TableFlags =
			ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_NoKeepColumnsVisible | ImGuiTableFlags_PadOuterX;
	...
	void InspectorPanel::DrawComponent(const char* name, Fn&& fn, bool removable)
	{
		...
		if (m_Context->m_Registry->has<ComponentType>(m_SelectionContext))
		{
			...
			bool open =
				ImGui::CollapsingHeader(name, ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_FramePadding | ImGuiTreeNodeFlags_AllowOverlap);
			ImGui::PopStyleVar();
			...
			if (ImGui::BeginPopupContextItem("ComponentSettings",
					ImGuiPopupFlags_NoOpenOverExistingPopup | ImGuiPopupFlags_MouseButtonLeft))
			{
