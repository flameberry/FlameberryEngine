...
namespace Flameberry {
	...
	public:
		...
		void OnUIRender();
		bool DrawMapControls(const char* label, bool& mapEnabledVar,
			Ref<Texture2D>& map);

	private:
		static constexpr ImGuiTableFlags s_TableFlags =
			ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_NoKeepColumnsVisible;
