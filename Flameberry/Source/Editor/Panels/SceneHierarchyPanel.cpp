...
namespace Flameberry {
	...
	void SceneHierarchyPanel::OnUIRender()
	{
		...
		UI::InputBox("##SceneHierarchySearchBar", width, m_SearchInputBuffer, 256,
		...
		{
			ImGui::TableSetupColumn(ICON_LC_TAG " Label",
				ImGuiTableColumnFlags_WidthStretch);
			ImGui::TableSetupColumn("Type", ImGuiTableColumnFlags_WidthFixed,
				ImGui::GetWindowWidth() / 4.0f);
		...
		ImGui::BeginChild("##EntityList", ImVec2(-1, -1), false,
		...
		ImRect windowRect(ImGui::GetWindowPos(),
			ImVec2(ImGui::GetWindowPos().x + ImGui::GetWindowSize().x,
				ImGui::GetWindowPos().y + ImGui::GetWindowSize().y));
		if (ImGui::BeginDragDropTargetCustom(windowRect,
				ImGui::GetID("##EntityList")))
		{
			if (const ImGuiPayload* payload =
					ImGui::AcceptDragDropPayload("FBY_SCENE_HIERARCHY_ENTITY_NODE"))
			{
				const fbentt::entity payloadEntity =
					*((const fbentt::entity*)payload->Data);
	...
	void SceneHierarchyPanel::DrawEntityNode(fbentt::entity entity)
	{
		...
		{
			// TODO: Maybe some optimisation to not search again if the input string is
			// same
		...
		int treeNodeFlags = (isSelected ? ImGuiTreeNodeFlags_Selected : 0) | ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_FramePadding;
		...
		ImGui::PushStyleColor(ImGuiCol_Header,
			Theme::AccentColor); // Main Accent Color
		...
		bool open = ImGui::TreeNodeEx((const void*)(uint64_t)entity, treeNodeFlags,
			"%s", tag.c_str());
		...
		{
			ImGui::SetDragDropPayload("FBY_SCENE_HIERARCHY_ENTITY_NODE", &entity,
				sizeof(entity), ImGuiCond_Once);
		...
		if (open)
		{
			...
			{
				fbentt::entity child =
					m_Context->m_Registry->get<RelationshipComponent>(entity).FirstChild;
				while (child != fbentt::null)
				{
					auto temp = m_Context->m_Registry->get<RelationshipComponent>(child)
									.NextSibling;
	...
	void SceneHierarchyPanel::DrawCreateEntityMenu(fbentt::entity parent)
	{
		if (ImGui::BeginMenu(ICON_LC_PLUS "\tCreate"))
		{
			...
			{
				auto entity =
					m_Context->CreateEntityWithTagAndParent("StaticMesh", parent);
			...
			if (ImGui::BeginMenu("Light"))
			{
				...
				{
					auto entity =
						m_Context->CreateEntityWithTagAndParent("Sky Light", parent);
					m_Context->m_Registry->emplace<SkyLightComponent>(entity);
					m_SelectionContext = entity;
				}
				if (ImGui::MenuItem(ICON_LC_SUN "\tDirectional Light"))
				{
					auto entity = m_Context->CreateEntityWithTagAndParent(
						"Directional Light", parent);
				...
				{
					auto entity =
						m_Context->CreateEntityWithTagAndParent("Point Light", parent);
