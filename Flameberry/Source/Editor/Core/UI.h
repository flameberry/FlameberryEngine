...
namespace Flameberry {
	...
	public:
		...
		static void Vec3Control(const std::string& label, glm::vec3& value,
			float defaultValue, float dragSpeed,
			float availWidth);
		static bool AlignedButton(const char* label,
			const ImVec2& size = ImVec2(0.0f, 0.0f),
			float alignment = 0.5f);
		static void InputBox(const char* label, const float width, char* inputBuffer,
			const uint32_t inputLength,
			const char* inputHint = (const char*)__null);

		static void OpenSelectionWidget(const char* label);
		static bool BeginSelectionWidget(const char* label, char* inputBuffer,
			const uint32_t inputLength);
		...
		static bool ContentBrowserItem(const std::filesystem::path& filepath,
			float size, const Ref<Texture2D>& thumbnail,
			ImVec2& outItemSize,
